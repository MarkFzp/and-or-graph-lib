...
var DotGraph, XDotGraph, astToStr, __hasProp = {}.hasOwnProperty, __extends = function(
		child, parent) {
...
astToStr = function(ast, indentLevel, indentChar) {
	...
	escape = function(s) {
		...
		if (/^[a-zA-Z0-9]+$/.test(s)
				&& !/^(graph|digraph|subgraph|node|edge|strict)$/.test(s)) {
			return s;
		} else {
			return "\"" + (('' + s).replace('"', '\\"')) + "\"";
		}
...
/*
 * # Takes in an AST of the dot/xdot file format # and produces a graph object
 * where nodes/edges/subgraphs # may be queried for attributes
 */
...
DotGraph = (function() {
	...
	/*
	 * # Light object to hold nodes and attributes of subgraphs. # This is
	 * really just a container and doesn't have any processing capabilities
	 */
	...
	DotGraph.prototype.walk = function(ast) {
		var getAllNodes, walk, _this = this;
		if (ast == null) {
			ast = this.ast;
		}
		walk = function(tree, state, currentParentGraph) {
			...
			case 'subgraph':
				...
				currentParentGraph = new DotSubgraph(tree.id || null,
						tree.type, currentParentGraph);
			...
			case 'node_stmt':
				...
				mergeLeftOverried(_this.nodes[id].attrs,
						attrListToObj(tree.attr_list));
				mergeLeftNoOverried(_this.nodes[id].attrs, state.node);
				currentParentGraph.nodes[id] = true;
				break;
			case 'attr_stmt':
				mergeLeftOverried(state[tree.target],
						attrListToObj(tree.attr_list));
				break;
			case 'edge_stmt':
				...
				for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
					...
					for (_l = 0, _len3 = heads.length; _l < _len3; _l++) {
						...
						for (_m = 0, _len4 = tails.length; _m < _len4; _m++) {
							t = tails[_m];
							edge = [ h, t ];
							attrs = mergeLeftNoOverried(
									attrListToObj(tree.attr_list), state.edge);
 ...
 * attributes like pos, width, height, etc. into the appropriate javascript
 * types. # # All attributes are normalized to pixels for easier drawing.
 */
...
XDotGraph = (function(_super) {
	...
	XDotGraph.prototype.parseAttr = function(attr, val) {
		...
		case 'pos':
			if (val.charAt(0) === 'e') {
				/*
				 * val = toFloatList(val) controlPoints = [] # arrow pos are of
				 * the form "'e',startx,starty, <triplets of bzCurve xy-coords>,
				 * arrowTargetx, arrowTargety" i = 2 while i + 6 < val.length
				 * controlPoints.push val.slice(i,i+6) i += 6 return {type:
				 * 'edge', origin: val[1..2], controlPoints: controlPoints,
				 * arrow: val.slice(-4)}
				 */
